...
**Verified default branch:** `master` (head `a5adb31`). *Rename to `main` is a pending
...
- **M4 (full Listen Mode) is merged** (PR #8). All 6 presets are demoable without a
  guitar or mic: an `AudioBufferSourceNode` loop controller replaces the old hidden
  media-element player, with lazy fetch/decode, URL-keyed buffer caching, pause/resume
  offsets, retry, observable state, and mobile Listen-only behavior. Listen audio routes
  through `engine.listenInput` (entering at NoiseGate, bypassing live-interface gain per
  PRD §4); Play and Listen are mutually exclusive. A fresh review's P1 (a pending
  microphone permission could race Listen and leave both a live mic source and the Listen
  riff feeding the graph) was fixed with generation-counter cancellation in Play Mode
  (`src/ui/playModeController.mjs`, `playMode.isStarting()`), covered by a CI regression
  test (`test/play-mode.test.mjs`).
- `npm test` (preset schema validator, the two engine-invariant guards, and the Listen +
  Play controller tests) is green.
...
| M4 — Listen Mode (full) | **Done** | Merged via PR #8. `AudioBufferSourceNode` loop controller for all 6 presets (lazy decode + URL-keyed cache, pause/resume, retry, observable state), routed through `engine.listenInput`; Play/Listen mutually exclusive; mobile Listen-only. Review P1 (pending-mic-permission race vs Listen) fixed with generation-counter cancellation in Play Mode + `test/play-mode.test.mjs`. Physical mobile Safari/Chrome playback remains an owner-only acceptance check. |
| M5 — Ship | **In progress** | `claude/m5-ship` (this PR): deploy-readiness slice — dashboard/README refresh, asset-budget audit (initial ≈103 KB, functional payload ≈1.1 MB, both ≪ 8 MB), repo hygiene, and Cloudflare Pages config + owner runbook. **Owner-only remainder before M5 is Done:** connect the Cloudflare Pages account (+ `master`→`main` rename), replace placeholder IR/riff with owner recordings (legal gate for public launch), the by-ear `draft`→`verified` tuning pass, and a formal Lighthouse pass. |
...
|---|---|---|---|---|
| Shared-core / integration | Claude | M5 — Ship (deploy-readiness slice) | `claude/m5-ship` | Docs/config/hygiene only; no engine/voicing/preset/audio changes. M4 (last occupant) merged via PR #8. |
...
| #8 | M4 (full Listen Mode): `AudioBufferSourceNode` loop controller for all 6 presets (lazy decode + URL-keyed cache, pause/resume, retry, observable state), routed through `engine.listenInput`; Play/Listen mutually exclusive; mobile Listen-only. A fresh review's P1 (pending-mic-permission race vs Listen) was fixed with generation-counter cancellation in Play Mode + a `test/play-mode.test.mjs` CI regression. No preset/schema/engine/audio changes. |
...
2. Open the repository's **Pull requests** page for live work (the M5 deploy-readiness
   PR from `claude/m5-ship` is the one open as of this writing).
